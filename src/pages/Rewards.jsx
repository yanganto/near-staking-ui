...
const Rewards = ({ wallet, isSignedIn }) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const mp = await fetch(nearConfig.backendUrl + 'pools', requestOptions)
        .then(async (response) => {
        ...
        .then((data) => {
          ...
          for (const v of data.myPools) {
            if (v.owner_id === wallet.accountId) pools.push(v.pool_id);
            else addedPools.push(v.pool_id);
          }
          return pools;
        })
        .catch((error) => {
      ...
      const kp = await fetch(
        ...
        .then((data) => {
          ...
          for (const v of data) {
            if (
              v.is_enabled &&
              !mp.includes(v.account_id) &&
              !addedPools.includes(v.account_id)
            )
              pools.push(v.account_id);
          }
          return pools;
        })
        .catch((err) => {
        ...
        <>
          <Box align="right" sx={{ m: 2 }}>
            ...
            <Button
              ...
              variant={group === '%Y-%m-%d' ? 'contained' : 'outlined'}
              size="small"
              sx={{ ml: 1 }}
            >
              Day
            ...
            <Button
              ...
              size="small"
              sx={{ ml: 1 }}
              variant={group === '%Y-%m' ? 'contained' : 'outlined'}
            >
              Month
            </Button>
            <Button
              ...
              size="small"
              sx={{ ml: 1 }}
              variant={group === '%Y' ? 'contained' : 'outlined'}
            >
              Year
            </Button>
          </Box>
